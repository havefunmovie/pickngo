...
//            dd($this->data);
            ...
            $dimensions->getUnitOfMeasurement()->setCode($this->data['package']['dimen-type']);
            $dimensions->setHeight($this->data['package']['height'])->setLength($this->data['package']['length'])->setWidth($this->data['package']['width']);
...
//            return response($confirm); // Confirm holds the digest you need to accept the result
    ...
    {
        $data = $this->data;
        return view('livewire.admin.agent.parcel.details.get-quote', compact('data'));
    }
}
