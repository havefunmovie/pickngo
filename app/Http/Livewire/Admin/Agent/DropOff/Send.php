...
class Send extends Component
{
    ...
    public function saveSendTrackingNumber()
    {
        ...
        $result = Dropoff::where('tracking_number',$this->send_tracking_number)->count();
        if($result <= 0)
        {
            $this->error = 'This barcode is not existing in DROPOFF database';
    ...
    public function updateTrackingNumberStatus()
    {
        ...
        if(($this->driver_id) && ($this->send_tracking_numbers != null))
        {
            foreach($this->send_tracking_numbers as $send_tracking_number){
                Dropoff::where('tracking_number',$send_tracking_number)->update(['status' => 'Done' , 'dropoff_agent_id' => $this->driver_id ,'note' => $this->note , 'send_at' => $date->format('Y-m-d H:i:s')]);
            }
    ...
    public function render()
    {
        ...
        $total_qty = Dropoff::where('status','Pending')->count();
        return view('livewire.admin.agent.drop-off.send',compact('drivers','total_qty'))->layout('livewire.admin.master');
    }
}