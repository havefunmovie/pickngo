    ...
    public function mount($data, $apiKey) {
        ...
        if ($data) {
            if ($this->label = $this->createLabel()) {
                $label = $this->label;
                if ($data['country_to'] == $data['country_from']) {
                    $this->hasNotInvoice = true;
                }

//                $this->label = "<img src='{$this->label}' style='transform:rotate(90deg);'/>";
                ...
                $tracking_number = $trak_num->generateBarcodeNumber('parcel');

                OrderParcel::where('id', $data['id'])->update([
                    'tracking_code' => $data['trackingNumber'],
                    'tracking_code_1' => $tracking_number,
                    'label' => $imageName,
                ]);

                Transactions::where('parcel_id', $data['id'])->update([
                    'status' => 'successful'
                ]);

                Session::put('image', $data['id']);
                Session::put('label-type', 'parcel');
                Session::put('invoice', $data['id']);
                Session::put('invoice-type', 'parcel');
            }
        }
    }
...
//                'dimen_type' => $params['package']['type'],
...
//        }
    ...
    {
        return view('livewire.home.panel.services.parcel-content.parcel.get-label');
    }
...
//            dd($this->data);
            ...
            $invoiceLineTotal->setCurrencyCode($this->data['currency']);
            $invoiceLineTotal->setMonetaryValue($this->data['price']);
