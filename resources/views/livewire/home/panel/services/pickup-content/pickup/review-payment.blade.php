    ...
    <div class="accordion accordion-flush" id="accordionFlushExample">
        <div class="accordion-item border-0 px-3 py-2 mt-3 {{ $shipDetail == 1 ? 'shadow-none' : 'shadow' }}">
            ...
            <div class="accordion-collapse row {{ $shipDetail == 1 ? 'show' : 'collapse' }}">
                <div class="col-md-6">
                    ...
                    <div class="row">
                        <div class="col">
                            <h6>{{ __('Contact Details') }}</h6>
                            <p class="text-secondary">
                                @isset($data)
                                    <span class="mdi mdi-account"></span> {{ $from['company'] }} <br>
                                    <span class="mdi mdi-email"></span> {{ $from['email'] }} <br>
                                    <span class="mdi mdi-phone"></span> {{ $from['phone'] }}
                                @endisset
                            </p>
                        </div>
                        <div class="col">
                            ...
                            <p class="text-secondary">
                                @isset($data)
                                <span class="mdi mdi-google-maps"></span> {{ $from['address1'] }} <br>
                                {{ $from['city'].', '. $from['province'].', '. $from['country']}} <br>
                                {{ $from['postal'] }}
                                @endisset
                            </p>
                        </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <p class="text-secondary">
                                @isset($data)
                                    <span class="mdi mdi-store"></span> {{ $to['company'] }} <br>
                                    <span class="mdi mdi-phone"></span> {{ $to['phone'] }}
                                @endisset
                            </p>
                        </div>
                        <div class="col">
                            ...
                            <p class="text-secondary">
                                @isset($data)
                                <span class="mdi mdi-google-maps"></span> {{ $to['address1'] }} <br>
                                    {{ $to['city'].', '. $to['province'] .', '. $to['country']}} <br>
                                    {{ $to['postal'] }}
                                @endisset
                            </p>
                        </div>
                            ...
                            @if(count($mServiceSummary))
                               @foreach($mServiceSummary as $row)
                                    <tr>
                                        <td>{{ 1 }}</td>
                                        <td>{{$row['name']}}</td>
                                        <td>{{$final_payable = $row['cost']}} CAD</td>
                                    </tr>
                               @endforeach
                            @endif
                        </table>
                        ...
                        <table class="table table-striped mt-3" width="100%">
                            ...
                            <tr>
                                @isset($data)
                                    ...
                                    <td class="text-secondary">{{ $package['length'] . 'x' . $package['width'] . 'x' . $package['height'] . ' (INCH)' }}</td>
                                    <td class="text-secondary">{{ $package['weight'] . ' (LBS)' }}</td>
                                    <td class="text-secondary">{{ __('0 ($)') }}</td>
                                    <td class="fw-bolder text-primary">{{ __('') }}</td>
                                @endisset
                            </tr>
                        </table>
                    </div>
        ...
        <div class="accordion-item border-0 px-3 py-2 mt-3 shadow-none">
            ...
            <div class="accordion-collapse show" >
                <livewire:home.panel.services.pickup-content.pickup.payment />
            </div>
        </div>
    </div>
</div>
